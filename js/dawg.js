...
var Dawg = function () {
    ...
    _createClass(Dawg, [{
        ...
        key: 'solve',
        value: function solve(gridLetters) {

            function recursiveSolve(node, word, i, j, grid, visited, words) {
                node = node.edges.get(grid[i][j]);
                ...
                for (let row = i-1; row <= i+1; row++) {
                    for (let col = j-1; col <= j+1; col++) {
                        if (row > -1 && col > -1 && row < grid.length && col < grid.length) {
                            if (!visited[row][col]) {
                                recursiveSolve(node, word, row, col, grid, visited, words);
                            }
                        }
                    }
                }
            ...
            for (let i = 0; i < n; i++) {
                for (let j = 0; j < n; j++) {
                    recursiveSolve(this._root, "", i, j, grid, visited, words);
                }
            }
            return words;
        }
        ...
        value: function has(path) {
            return this.longest2(path) === path.length;
        }
